...
<html lang="en">
  <head>
    ...
    <!-- Title -->
    <title>KmEmpire - ONL_SHOPPING || All Product Page</title>
    ...
    <div class="header-area" id="headerArea">
      <div class="container h-100 d-flex align-items-center justify-content-between rtl-flex-d-row-r">
        ...
        <div class="page-heading">
          <h6 class="mb-0">Shop Grid - New Arrivals Products</h6>
        </div>
        <!-- Filter Option-->
...
<?php
include("db_con.php"); // Ensure database connection

$sortBy = "id"; // Default sorting column

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['selectProductCatagory'])) {
        $selectedOption = $_POST['selectProductCatagory'];

        // Define sorting based on selected option
        switch ($selectedOption) {
            case "1":
                $sortBy = "created_at DESC"; // Newest products first
                break;
            case "2":
                $sortBy = "popularity DESC"; // Most popular first
                break;
            case "3":
                $sortBy = "ratings DESC"; // Highest ratings first
                break;
            default:
...
// Fetch products with the selected sorting method
$sql = "SELECT * FROM add_product ORDER BY $sortBy";
$result = mysqli_query($con, $sql);
?>
    <div class="select-product-catagory">
        <select class="right small border-0" id="selectProductCatagory" name="selectProductCatagory" aria-label="Default select example" onchange="this.form.submit()">
          ...
          <div class="row g-2 rtl-flex-d-row-r">
            ...
            <?php
                    include('db_con.php');
                    $sql= "SELECT * FROM add_product WHERE prod_sec = 'New Arrivals'";
                    $result= mysqli_query($con, $sql);
            ...
            <div class="col-6 col-md-4">
              <div class="card product-card">
                <div class="card-body">
                  <!-- Badge--><span class="badge rounded-pill badge-warning"><?php echo $row['prod_stat'];?></span>
